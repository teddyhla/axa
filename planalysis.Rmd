...
<style>
@import url('https://fonts.googleapis.com/css2?family=Fira+Sans&display=swap" rel="stylesheet');
body {
...
- NOTE : No gold standard monitoring profiles.
...
| Method       | **Core**      | Rosendaal            | Traditional            | Cross-sectional                       |
|--------------|---------------|----------------------|------------------------|---------------------------------------|
...
- NOTE :Calculations adjusted for patients with PE (n = 2 , but have different threshold.)
...
#this shows that 14 patients in wave 1 and wave 2 COVID cohorts never were used to monitor AXA. OR IF they did, they never reach the range.
...
dfaxa <- clab %>% filter(axarange == "in") %>% group_by(mrn) %>% slice_min(dtm) %>% ungroup()
dfapt <-clab %>% filter(apttrrange == "in") %>% group_by(mrn) %>% slice_min(dtm) %>% ungroup()
```
