...
think about begininig nd end of ecmo run.
think about other complications e.g., haemorrhagic complications

#deciding which feature to add
- day on ecmo . hypothesis = we do need to as we transfuse more at the begining.

#lets look at total products for ecmo run 

note median ecmo run time = 14 days
1st quartile 8 days
2nd quartile 14 days
3rd quartile 25.75 days
4th quartile 100 days
```{r}
e2 <- dfprd %>% 
        group_by(ecmod) %>%
        summarise(
...
thus we can set 1 day as a unit of 'window'
tranfusion in this window "yes" "no"
...
so this below code works. 
now lets append this with total no of products transfused
```{r}
...
how do we analyse  day 1? do we analyse from day 2?
may be we cant analyse that. 
...
on average how many days do the patient got transfused. 