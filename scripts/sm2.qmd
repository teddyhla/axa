...
```{r}
sjPlot::tab_model(mox02, title = "Multivariable analysis of VGR")
```

### Interpretation

Variability of anticoagulation is a measure of quality of anticoagulation, with lower Variability results in better control of anticoagulation. For 1 unit increase in lactate, our variability increases by 10.6% membership in anti Xa decreases variability by 86%.
...
```{r}
sjPlot::tab_model(bf, title = "Rate of any bleeding or thrombotic events")
```
...
```{r}
tab_model(h23, title = "Only haemorrhagic complication events")
```
...
```{r}
sjPlot::tab_model(cf, title = "Thrombotic Complications only")
```
...
```{r}
sjPlot::tab_model(xf,title = "ECMO circuit change analysis")
```
...
```{r}
sjPlot::tab_model(hn5, title = "Cumulative dose of heparin - log transformed")
```
...
```{r}
sjPlot::tab_model(r51, title = "Analysis of heparin prescription changes")
```
...
```{r}
sjPlot::tab_model(bp42, title = "Total blood products transfused")
```
...
2. references need sort out
3. update some more of the table
4. answer andy doyles answers
